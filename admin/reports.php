...
$reports['untagged_albums'] = array('title'=>'Untagged Albums','sql'=>'select id,albums.name,count(album_tags.id) as tag_count from albums left join album_tags on albums.id=album_FK group by albums.id order by albums.name having tag_count=0');
