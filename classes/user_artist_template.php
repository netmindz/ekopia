...
class user_artist_template
{
	...
	function add($addslashes=0) {
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->user_FK)."', '".addslashes($this->artist_FK)."')";
		}else{
			$raw_sql.= " VALUES ('$this->user_FK', '$this->artist_FK')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`user_FK`='".addslashes($this->user_FK)."', `artist_FK`='".addslashes($this->artist_FK)."'";
		}else{
			$raw_sql.= "`user_FK`='$this->user_FK', `artist_FK`='$this->artist_FK'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		elseif ($addslashes)
			$sql = "UPDATE user_artists SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE user_artists SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM user_artists WHERE 1
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"user_artist",$this->id);
