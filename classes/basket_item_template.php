...
class basket_item_template
{
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for basket_item->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->basket_id != (int)$this->basket_id && $this->basket_id!='NOW()' && $this->basket_id!='NULL'){
			trigger_error("wrong type for basket_item->basket_id",E_USER_WARNING);
			settype($this->basket_id,"int");
		}//IF


		if($this->item_id != (int)$this->item_id && $this->item_id!='NOW()' && $this->item_id!='NULL'){
			trigger_error("wrong type for basket_item->item_id",E_USER_WARNING);
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->basket_id)."', '".addslashes($this->type)."', '".addslashes($this->item_id)."')";
		}else{
			$raw_sql.= " VALUES ('$this->basket_id', '$this->type', '$this->item_id')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`basket_id`='".addslashes($this->basket_id)."', `type`='".addslashes($this->type)."', `item_id`='".addslashes($this->item_id)."'";
		}else{
			$raw_sql.= "`basket_id`='$this->basket_id', `type`='$this->type', `item_id`='$this->item_id'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE basket_items SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE basket_items SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE basket_items SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT basket_items.* FROM basket_items ";
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("basket_item", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"basket_item",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['basket_item'][$property]['max']) && $CONF['basket_item'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['basket_item'][$property]['max']) && $CONF['basket_item'][$property]['max']){
					$min = ($CONF['basket_item'][$property]['min'])? $CONF['basket_item'][$property]['min'] : 0;
					$step = ($CONF['basket_item'][$property]['step'])? $CONF['basket_item'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['basket_item'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['basket_item'][$property]['size']))
					$size = $CONF['basket_item'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['basket_item'][$property]['past']))
						$past = $CONF['basket_item'][$property]['past'];
					
					if (isset($CONF['basket_item'][$property]['future']))
						$future = $CONF['basket_item'][$property]['future'];
