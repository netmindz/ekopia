<?
class track_tag_template
{
	var $id, $track_FK, $tag_FK;
	...
	function __construct()
	{
		...
		$this->_field_descs['track_FK'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
		$this->_field_descs['tag_FK'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for track_tag->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->track_FK != (int)$this->track_FK && $this->track_FK!='NOW()' && $this->track_FK!='NULL'){
			trigger_error("wrong type for track_tag->track_FK",E_USER_WARNING);
			settype($this->track_FK,"int");
		}//IF


		if($this->tag_FK != (int)$this->tag_FK && $this->tag_FK!='NOW()' && $this->tag_FK!='NULL'){
			trigger_error("wrong type for track_tag->tag_FK",E_USER_WARNING);
			settype($this->tag_FK,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO track_tags (`track_FK`, `tag_FK`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->track_FK)."', '".addslashes($this->tag_FK)."')";
		}else{
			$raw_sql.= " VALUES ('$this->track_FK', '$this->tag_FK')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		$raw_sql  = "UPDATE track_tags SET ";
		if($addslashes) {
			$raw_sql.= "`track_FK`='".addslashes($this->track_FK)."', `tag_FK`='".addslashes($this->tag_FK)."'";
		}else{
			$raw_sql.= "`track_FK`='$this->track_FK', `tag_FK`='$this->tag_FK'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE track_tags SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE track_tags SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE track_tags SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM track_tags WHERE 1
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT track_tags.* FROM track_tags ";
...
//			$this = set_properties($this, $tmp, $addslashes,"get");
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("track_tag", $this->id, array_keys($value));
	...
	function _setPropertiesLinkages($parent_class, $pk, $values)
	{
		...
		$sql = "DELETE FROM track_tags WHERE {$parent_class}_FK = '$pk'\n";
		$sql .= " AND {$child_class}_FK NOT IN(" . implode("," , $values) .")";
		$this->database->query($sql);
	
		$sql = "INSERT INTO track_tags\n";
		$sql .= " (`{$parent_class}_FK`, `{$child_class}_FK`) ";
		$sql .= "select '$pk',$child_table." . $child_table_pk. " from $child_table left join track_tags on ({$parent_class}_FK = '$pk' AND {$child_class}_FK = $child_table.$child_table_pk) where track_tags.id is null and $child_table.$child_table_pk in (" . implode(",",$values) . ")";
		$this->database->query($sql);
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"track_tag",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['track_tag'][$property]['max']) && $CONF['track_tag'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['track_tag'][$property]['max']) && $CONF['track_tag'][$property]['max']){
					$min = ($CONF['track_tag'][$property]['min'])? $CONF['track_tag'][$property]['min'] : 0;
					$step = ($CONF['track_tag'][$property]['step'])? $CONF['track_tag'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['track_tag'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['track_tag'][$property]['size']))
					$size = $CONF['track_tag'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['track_tag'][$property]['past']))
						$past = $CONF['track_tag'][$property]['past'];
					
					if (isset($CONF['track_tag'][$property]['future']))
						$future = $CONF['track_tag'][$property]['future'];
