...
class track_tag_template
{
	...
	function add() {
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->track_FK)."', '".$this->database->escape($this->tag_FK)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`track_FK`='".$this->database->escape($this->track_FK)."', `tag_FK`='".$this->database->escape($this->tag_FK)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			...
			$sql = "UPDATE track_tags SET $fieldname = '".$this->database->escape($this->$fieldname)."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM track_tags WHERE id = '".$this->database->escape($id)."' ";
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"track_tag",$this->id);
