...
class artist_template
{
	...
	function add($addslashes=0) {
		...
		if($this->image_id != (int)$this->image_id && $this->image_id!='NOW()' && $this->image_id!='NULL'){
			trigger_error("wrong type for artist->image_id",E_USER_WARNING);
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->name)."', '".addslashes($this->website)."', '".addslashes($this->image_id)."', '".addslashes($this->summary)."')";
		}else{
			$raw_sql.= " VALUES ('$this->name', '$this->website', '$this->image_id', '$this->summary')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`name`='".addslashes($this->name)."', `website`='".addslashes($this->website)."', `image_id`='".addslashes($this->image_id)."', `summary`='".addslashes($this->summary)."'";
		}else{
			$raw_sql.= "`name`='$this->name', `website`='$this->website', `image_id`='$this->image_id', `summary`='$this->summary'";
		}//IF
