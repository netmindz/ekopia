...
class user_template
{
	...
	function add() {
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->username)."', '".$this->database->escape($this->password)."', '".$this->database->escape($this->email)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`username`='".$this->database->escape($this->username)."', `password`='".$this->database->escape($this->password)."', `email`='".$this->database->escape($this->email)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			...
			$sql = "UPDATE users SET $fieldname = '".$this->database->escape($this->$fieldname)."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM users WHERE id = '".$this->database->escape($id)."' ";
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"user",$this->id);
