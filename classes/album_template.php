...
class album_template
{
	...
	function add() {
		...
		if($this->artist_id != (int)$this->artist_id && $this->artist_id!='NOW()' && $this->artist_id!='NULL'){
			trigger_error("wrong type for album->artist_id",E_USER_WARNING);
			settype($this->artist_id,"int");
		}//IF


		if($this->label_id != (int)$this->label_id && $this->label_id!='NOW()' && $this->label_id!='NULL'){
			trigger_error("wrong type for album->label_id",E_USER_WARNING);
			settype($this->label_id,"int");
		}//IF


		if($this->release_year != (int)$this->release_year && $this->release_year!='NOW()' && $this->release_year!='NULL'){
			trigger_error("wrong type for album->release_year",E_USER_WARNING);
			settype($this->release_year,"int");
		}//IF


		if($this->image_id != (int)$this->image_id && $this->image_id!='NOW()' && $this->image_id!='NULL'){
			trigger_error("wrong type for album->image_id",E_USER_WARNING);
			settype($this->image_id,"int");
		}//IF


		if($this->stock_count != (int)$this->stock_count && $this->stock_count!='NOW()' && $this->stock_count!='NULL'){
			trigger_error("wrong type for album->stock_count",E_USER_WARNING);
			settype($this->stock_count,"int");
		}//IF
