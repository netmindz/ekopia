...
class order_template
{
	...
	function __construct()
	{
		...
		$this->_field_descs['created'] = array ("type" => "timestamp", "gen_type" => "timestamp");
		$this->_field_descs['order_ref'] = array ("type" => "varchar(255)", "length" => "255", "gen_type" => "string");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for order->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->order_ref)."', '".addslashes($this->paypal_txn_id)."', '".addslashes($this->payment_status)."', '".addslashes($this->customer_email)."')";
		}else{
			$raw_sql.= " VALUES ('$this->order_ref', '$this->paypal_txn_id', '$this->payment_status', '$this->customer_email')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`order_ref`='".addslashes($this->order_ref)."', `paypal_txn_id`='".addslashes($this->paypal_txn_id)."', `payment_status`='".addslashes($this->payment_status)."', `customer_email`='".addslashes($this->customer_email)."'";
		}else{
			$raw_sql.= "`order_ref`='$this->order_ref', `paypal_txn_id`='$this->paypal_txn_id', `payment_status`='$this->payment_status', `customer_email`='$this->customer_email'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE orders SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE orders SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE orders SET $fieldname = '".$this->$fieldname."'";
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("order", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"order",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['order'][$property]['max']) && $CONF['order'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['order'][$property]['max']) && $CONF['order'][$property]['max']){
					$min = ($CONF['order'][$property]['min'])? $CONF['order'][$property]['min'] : 0;
					$step = ($CONF['order'][$property]['step'])? $CONF['order'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['order'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['order'][$property]['size']))
					$size = $CONF['order'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['order'][$property]['past']))
						$past = $CONF['order'][$property]['past'];
					
					if (isset($CONF['order'][$property]['future']))
						$future = $CONF['order'][$property]['future'];
