...
{
	var $id, $order_id, $item, $price, $type, $item_id, $delivery, $quantity, $shipped;
	...
	function __construct()
	{
		...
		$this->_field_descs['shipped'] = array ("type" => "int(1)", "length" => "1", "gen_type" => "int");
	...
	function add() {
		...
		if($this->quantity != (int)$this->quantity && $this->quantity!='NOW()' && $this->quantity!='NULL'){
			trigger_error("wrong type for line_item->quantity",E_USER_WARNING);
			settype($this->quantity,"int");
		}//IF


		if($this->shipped != (int)$this->shipped && $this->shipped!='NOW()' && $this->shipped!='NULL'){
			trigger_error("wrong type for line_item->shipped",E_USER_WARNING);
			settype($this->shipped,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO line_items (`order_id`, `item`, `price`, `type`, `item_id`, `delivery`, `quantity`, `shipped`)";
		
		$raw_sql.= " VALUES ('".$this->database->escape($this->order_id)."', '".$this->database->escape($this->item)."', '".$this->database->escape($this->price)."', '".$this->database->escape($this->type)."', '".$this->database->escape($this->item_id)."', '".$this->database->escape($this->delivery)."', '".$this->database->escape($this->quantity)."', '".$this->database->escape($this->shipped)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`order_id`='".$this->database->escape($this->order_id)."', `item`='".$this->database->escape($this->item)."', `price`='".$this->database->escape($this->price)."', `type`='".$this->database->escape($this->type)."', `item_id`='".$this->database->escape($this->item_id)."', `delivery`='".$this->database->escape($this->delivery)."', `quantity`='".$this->database->escape($this->quantity)."', `shipped`='".$this->database->escape($this->shipped)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	{
		if(!$order) $order = "";
		$select = "SELECT line_items.* FROM line_items ";
			...
			foreach ($properties as $key => $value) {
				if(isset($this->_field_descs[$key]['fk'])) {
					...
					if($this->_field_descs[$key]['gen_type'] == "many2many") {
	                        ...
	                        $child->_setPropertiesLinkages("line_item", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"line_item",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if (isset($matches[1]) && ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['line_item'][$property]['max']) && $CONF['line_item'][$property]['max']==1)) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['line_item'][$property]['max']) && $CONF['line_item'][$property]['max']){
					$min = ($CONF['line_item'][$property]['min'])? $CONF['line_item'][$property]['min'] : 0;
					$step = ($CONF['line_item'][$property]['step'])? $CONF['line_item'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['line_item'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['line_item'][$property]['size']))
					$size = $CONF['line_item'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['line_item'][$property]['past']))
						$past = $CONF['line_item'][$property]['past'];
					
					if (isset($CONF['line_item'][$property]['future']))
						$future = $CONF['line_item'][$property]['future'];
