...
class tag_template
{
	...
	function add() {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for tag->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->name)."', '".$this->database->escape($this->public)."')";
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE tags SET $fieldname = ".$this->$fieldname;
		else
			$sql = "UPDATE tags SET $fieldname = '".$this->database->escape($this->$fieldname)."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM tags WHERE id = '".$this->database->escape($id)."' ";
			...
			foreach ($properties as $key => $value) {
				if(isset($this->_field_descs[$key]['fk'])) {
					...
					if($this->_field_descs[$key]['gen_type'] == "many2many") {
	                        ...
	                        $child->_setPropertiesLinkages("tag", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"tag",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if (isset($matches[1]) && ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['tag'][$property]['max']) && $CONF['tag'][$property]['max']==1)) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['tag'][$property]['max']) && $CONF['tag'][$property]['max']){
					$min = ($CONF['tag'][$property]['min'])? $CONF['tag'][$property]['min'] : 0;
					$step = ($CONF['tag'][$property]['step'])? $CONF['tag'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['tag'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['tag'][$property]['size']))
					$size = $CONF['tag'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['tag'][$property]['past']))
						$past = $CONF['tag'][$property]['past'];
					
					if (isset($CONF['tag'][$property]['future']))
						$future = $CONF['tag'][$property]['future'];
