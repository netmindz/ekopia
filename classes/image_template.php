...
{
	var $id, $name, $width, $height, $size, $type, $data;
	...
	function __construct()
	{
		...
		$this->_field_descs['width'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for image->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->width != (int)$this->width && $this->width!='NOW()' && $this->width!='NULL'){
			trigger_error("wrong type for image->width",E_USER_WARNING);
			settype($this->width,"int");
		}//IF


		if($this->height != (int)$this->height && $this->height!='NOW()' && $this->height!='NULL'){
			trigger_error("wrong type for image->height",E_USER_WARNING);
			settype($this->height,"int");
		}//IF


		if($this->size != (int)$this->size && $this->size!='NOW()' && $this->size!='NULL'){
			trigger_error("wrong type for image->size",E_USER_WARNING);
			settype($this->size,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO images (`name`, `width`, `height`, `size`, `type`, `data`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->name)."', '".addslashes($this->width)."', '".addslashes($this->height)."', '".addslashes($this->size)."', '".addslashes($this->type)."', '".addslashes($this->data)."')";
		}else{
			$raw_sql.= " VALUES ('$this->name', '$this->width', '$this->height', '$this->size', '$this->type', '$this->data')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`name`='".addslashes($this->name)."', `width`='".addslashes($this->width)."', `height`='".addslashes($this->height)."', `size`='".addslashes($this->size)."', `type`='".addslashes($this->type)."', `data`='".addslashes($this->data)."'";
		}else{
			$raw_sql.= "`name`='$this->name', `width`='$this->width', `height`='$this->height', `size`='$this->size', `type`='$this->type', `data`='$this->data'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE images SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE images SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE images SET $fieldname = '".$this->$fieldname."'";
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("image", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"image",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['image'][$property]['max']) && $CONF['image'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['image'][$property]['max']) && $CONF['image'][$property]['max']){
					$min = ($CONF['image'][$property]['min'])? $CONF['image'][$property]['min'] : 0;
					$step = ($CONF['image'][$property]['step'])? $CONF['image'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['image'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['image'][$property]['size']))
					$size = $CONF['image'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['image'][$property]['past']))
						$past = $CONF['image'][$property]['past'];
					
					if (isset($CONF['image'][$property]['future']))
						$future = $CONF['image'][$property]['future'];
