...
class type_template
{
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for type->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->type_id != (int)$this->type_id && $this->type_id!='NOW()' && $this->type_id!='NULL'){
			trigger_error("wrong type for type->type_id",E_USER_WARNING);
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->type_id)."', '".addslashes($this->name)."')";
		}else{
			$raw_sql.= " VALUES ('$this->type_id', '$this->name')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`type_id`='".addslashes($this->type_id)."', `name`='".addslashes($this->name)."'";
		}else{
			$raw_sql.= "`type_id`='$this->type_id', `name`='$this->name'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE types SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE types SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE types SET $fieldname = '".$this->$fieldname."'";
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("type", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"type",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['type'][$property]['max']) && $CONF['type'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['type'][$property]['max']) && $CONF['type'][$property]['max']){
					$min = ($CONF['type'][$property]['min'])? $CONF['type'][$property]['min'] : 0;
					$step = ($CONF['type'][$property]['step'])? $CONF['type'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['type'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['type'][$property]['size']))
					$size = $CONF['type'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['type'][$property]['past']))
						$past = $CONF['type'][$property]['past'];
					
					if (isset($CONF['type'][$property]['future']))
						$future = $CONF['type'][$property]['future'];
