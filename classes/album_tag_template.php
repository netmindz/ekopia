<?
class album_tag_template
{
	var $id, $tag_FK, $album_FK;
	...
	function __construct()
	{
		...
		$this->_field_descs['album_FK'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for album_tag->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->tag_FK != (int)$this->tag_FK && $this->tag_FK!='NOW()' && $this->tag_FK!='NULL'){
			trigger_error("wrong type for album_tag->tag_FK",E_USER_WARNING);
			settype($this->tag_FK,"int");
		}//IF


		if($this->album_FK != (int)$this->album_FK && $this->album_FK!='NOW()' && $this->album_FK!='NULL'){
			trigger_error("wrong type for album_tag->album_FK",E_USER_WARNING);
			settype($this->album_FK,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO album_tags (`tag_FK`, `album_FK`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->tag_FK)."', '".addslashes($this->album_FK)."')";
		}else{
			$raw_sql.= " VALUES ('$this->tag_FK', '$this->album_FK')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`tag_FK`='".addslashes($this->tag_FK)."', `album_FK`='".addslashes($this->album_FK)."'";
		}else{
			$raw_sql.= "`tag_FK`='$this->tag_FK', `album_FK`='$this->album_FK'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE album_tags SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE album_tags SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE album_tags SET $fieldname = '".$this->$fieldname."'";
...
//			$this = set_properties($this, $tmp, $addslashes,"get");
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("album_tag", $this->id, array_keys($value));
	...
	function _setPropertiesLinkages($parent_class, $pk, $values)
	{
		...
		$sql = "DELETE FROM album_tags WHERE {$parent_class}_FK = '$pk'\n";
		$sql .= " AND {$child_class}_FK NOT IN(" . implode("," , $values) .")";
		$this->database->query($sql);
	
		$sql = "INSERT INTO album_tags\n";
		$sql .= " (`{$parent_class}_FK`, `{$child_class}_FK`) ";
		$sql .= "select '$pk',$child_table." . $child_table_pk. " from $child_table left join album_tags on ({$parent_class}_FK = '$pk' AND {$child_class}_FK = $child_table.$child_table_pk) where album_tags.id is null and $child_table.$child_table_pk in (" . implode(",",$values) . ")";
		$this->database->query($sql);
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"album_tag",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['album_tag'][$property]['max']) && $CONF['album_tag'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['album_tag'][$property]['max']) && $CONF['album_tag'][$property]['max']){
					$min = ($CONF['album_tag'][$property]['min'])? $CONF['album_tag'][$property]['min'] : 0;
					$step = ($CONF['album_tag'][$property]['step'])? $CONF['album_tag'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['album_tag'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['album_tag'][$property]['size']))
					$size = $CONF['album_tag'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['album_tag'][$property]['past']))
						$past = $CONF['album_tag'][$property]['past'];
					
					if (isset($CONF['album_tag'][$property]['future']))
						$future = $CONF['album_tag'][$property]['future'];
